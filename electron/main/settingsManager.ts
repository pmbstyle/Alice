...
export async function loadSettings(): Promise<AppSettings | null> {
  try {
    ...
    console.warn('Failed to load settings or settings file not found:', error.message);
    return null;
  }
}
...
export async function deleteSettingsFile(): Promise<void> {
  try {
    ...
    console.warn('Failed to delete settings file (it may not exist):', error.message);
  }
}